...
app = FastAPI(
    title="Compliance Monitor — Data Pipeline",
...
async def ingest_pdf(file: UploadFile = File(...)):
    if not pipeline:
        raise HTTPException(status_code=503, detail="Set ANTHROPIC_API_KEY and install anthropic pdfplumber")
...
def explain_violation(req: ViolationExplainRequest):
    if not explainer or not api_key:
        raise HTTPException(status_code=503, detail="LLM explainer unavailable. Set ANTHROPIC_API_KEY.")
...
if __name__ == "__main__":
    ...
    uvicorn.run("anushka_server:app", host="0.0.0.0", port=port, reload=False)
